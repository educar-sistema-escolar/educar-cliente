...
export function CatalogList<T extends Sport | DiningService>({ items, onToggle, showCapacity = false }: { items: T[]; onToggle: (item: T) => void; showCapacity?: boolean }) {
...
export function EnrollmentList<T extends { id: string; is_active: boolean }>({ items, empty, onDeactivate, render }: { items: T[]; empty: string; onDeactivate: (id: string) => void; render: (item: T) => ReactNode }) {
