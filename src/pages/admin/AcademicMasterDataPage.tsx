...
const emptyForms: Record<Resource, FormState> = {
  ...
  subjects: { code: '', name: '' },
  teachers: { first_name: '', last_name: '', email: '', phone: '', teacher_number: '' },
  students: { first_name: '', last_name: '', email: '', phone: '', student_number: '' },
};
...
export const AcademicMasterDataPage: React.FC<{ resource: Resource }> = ({ resource }) => {
  ...
  function startEdit(item: EducationalLevel | Course | Subject | Teacher | Student) {
    ...
    if (resource === 'teachers') {
      ...
      setForm({ first_name: teacher.person.first_name, last_name: teacher.person.last_name, email: teacher.person.email ?? '', phone: teacher.person.phone ?? '', teacher_number: teacher.teacher_number ?? '' });
    }
    if (resource === 'students') {
      const student = item as Student;
      setForm({ first_name: student.person.first_name, last_name: student.person.last_name, email: student.person.email ?? '', phone: student.person.phone ?? '', student_number: student.student_number ?? '' });
    }
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      if (resource === 'teachers') {
        const input = { first_name: form.first_name.trim(), last_name: form.last_name.trim(), email: form.email.trim() || undefined, phone: form.phone.trim() || undefined, teacher_number: form.teacher_number.trim() || undefined };
      ...
      if (resource === 'students') {
        const input = { first_name: form.first_name.trim(), last_name: form.last_name.trim(), email: form.email.trim() || undefined, phone: form.phone.trim() || undefined, student_number: form.student_number.trim() || undefined };
        if (!input.first_name || !input.last_name) throw new Error('Nombre y apellido son obligatorios.');
        ...
        {isLoading ? <p className="py-12 text-center text-xs text-edu-muted">Cargando datos...</p> : visibleItems.length === 0 ? <p className="py-12 text-center text-xs text-edu-muted">No hay registros.</p> : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-600">
              <thead><tr className="border-b border-edu-border bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-edu-muted"><th className="px-3 py-3">Nombre</th><th className="px-3 py-3">Código / identificación</th><th className="px-3 py-3">Estado</th><th className="px-3 py-3 text-right">Acciones</th></tr></thead>
              <tbody className="divide-y divide-edu-border/60">
                {visibleItems.map((item) => {
                  ...
                  const code = resource === 'teachers' ? (item as Teacher).teacher_number : resource === 'students' ? (item as Student).student_number : (item as EducationalLevel | Course | Subject).code;
